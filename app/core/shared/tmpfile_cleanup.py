...
def release_file_lock(handle: Optional[TextIO]) -> None:
    ...
    try:
        ...
        logger.exception("[TMP] Failed to release file lock")
    finally:
        handle.close()
...
async def cleanup_loop(
    ...
    """
    logger.debug(f"[TMP] Cleanup loop started: {name}")
    try:
        while not stop_event.is_set():
            try:
                await cleanup_fn(time.time())
            except Exception:
                logger.exception("[TMP] Cleanup loop failed")
            try:
                await asyncio.wait_for(stop_event.wait(), timeout=interval_seconds)
            except asyncio.TimeoutError:
                continue
    finally:
        logger.debug(f"[TMP] Cleanup loop stopped: {name}")
